...
fixed_df <- data_frame(type='discrete', lb=-4, ub=Inf, minconc=0.001, maxconc=30,
                       ndoses=c(7,10), nreps=c(1,3), assay=c('7pt_1rep', '10pt_3rep'),
                       sd_prop=0.3, n=50, prop=0.5, beta=1)
varying_df <- crossing(sd=c(0.4, 1), mu=c(0,1,2,3), sd_add=c(0.15, 0.5)) %>%
